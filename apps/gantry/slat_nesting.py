...
# Offset of cardboard relative to gantry-centered origin (0,0)
...
# Coordinates are in the current VIEW frame, centered on gantry origin.
MANUAL_LAYOUT = [
    ("XY_left_02", -80.0, -40.0, 0.0),
    ("XY_right_03", 10.0, -30.0, 0.0),
    ("XZ_left_01", 80.0, -20.0, 90.0),
]
...
def compute_cardboard_mm_scale(sheet, target_cardboard_width_mm):
    ...
    cardboard_length_view_units = span_x
    ...
    print(">>> Interpreting cardboard width from current view_y span:", cardboard_width_view_units)
    print(">>> Interpreting cardboard length from current view_x span:", cardboard_length_view_units)
...
def compute_centering_translation_from_sheet(sheet, offset_x=0.0, offset_y=0.0):
    minx, miny, maxx, maxy = sheet.bounds
    cx = 0.5 * (minx + maxx)
    cy = 0.5 * (miny + maxy)
    dx = -cx + offset_x
...
def describe_cardboard_bounds(label, geom):
    minx, miny, maxx, maxy = geom.bounds
    span_x = maxx - minx
    span_y = maxy - miny

    print(f">>> {label} bounds: {geom.bounds}")
    print(f">>> {label} view-length span (x): {span_x}")
    print(f">>> {label} view-width span  (y): {span_y}")


def bounding_box_polygon(geom):
...
def auto_place_selected_slats(selected_slats, usable_region, cfg: AutoPlaceConfig):
    ...
    for slat_id, base_geom, _ in prepared:
        ...
        for rot_deg in cfg.rotations_deg:
            for x, y in grid_points(usable_region.bounds, cfg.search_step_x, cfg.search_step_y):
                candidate = place_geom(base_geom, x, y, rot_deg)
                if fits_in_region(candidate, usable_region, placed_geoms, cfg.part_gap):
                    placed.append((slat_id, candidate, (x, y, rot_deg), True, "auto"))
...
def draw_bounding_box(ax, geom, label="bbox"):
    bbox = bounding_box_polygon(geom)
    plot_polygon(ax, bbox, color="purple", linewidth=1.0, linestyle=":")
    minx, miny, maxx, maxy = bbox.bounds
    ax.text(minx, maxy + 6.0, label, color="purple", fontsize=8, ha="left", va="bottom")
...
def preview_interpreted_cardboard(sheet, holes, usable_region, title="Interpreted cardboard view"):
    ...
    if SHOW_BOUNDING_BOX:
        draw_bounding_box(ax, sheet, label="sheet bbox")
        draw_bounding_box(ax, usable_region, label="usable bbox")
...
def preview_selected_slats_on_cardboard(sheet, holes, usable_region, placed_slats):
    ...
    for slat_id, geom, pose, ok, note in placed_slats:
        ...
        ax.text(
            minx,
            maxy + 5.0,
            f"{slat_id} @ ({x:.0f}, {y:.0f}, {rot_deg:.0f}°)",
            fontsize=9,
            color="darkgreen",
            ha="left",
            va="bottom",
...
def main():
    ...
    describe_cardboard_bounds("raw_dxf_union", all_union)
    ...
    describe_cardboard_bounds("sheet_raw", sheet_raw)
    ...
    dx, dy = compute_centering_translation_from_sheet(
        sheet_mm,
    ...
    describe_cardboard_bounds("sheet_mm", sheet_mm)
    describe_cardboard_bounds("usable_region_mm", usable_region_mm)

    print(">>> Gantry-centered mm sheet bounds:", sheet_mm.bounds)
    print(">>> Gantry-centered usable region bounds:", usable_region_mm.bounds)

    preview_interpreted_cardboard(
        sheet_mm,
        holes_mm,
        usable_region_mm,
        title="Interpreted cardboard view (mm-calibrated, gantry-centered)",
    )
